...
def save_example_image(
    ...
    """Save a side-by-side comparison of reconstruction and ground truth with metrics.

    Args:
      recon: Reconstructed image, shape (H, W) or (H, W, 1), values in [0, 1].
      ground_truth: Ground truth image, same shape as recon, values in [0, 1].
      psnr: PSNR value for this image.
      ssim: SSIM value for this image.
      save_path: Full path to save the figure.
      method_name: Name of the method (e.g., 'fista_tv', 'projection').
    """
...
def evaluate(config, workdir, eval_folder="eval"):
    ...
    method_name = config.sampling.cs_solver.lower()
    ...
    test_imgs = test_imgs.reshape((jax.process_count(), -1, *test_imgs.shape[1:]))[
        jax.process_index()
    ]

    if "mar" in config.sampling.task:
        mar = True
        mar_data_dir = {"ct2d_320": "LIDC_320_MAR.npz", "ldct_512": "LDCT_MAR.npz"}[
            config.data.dataset
        ]
        ...
        metal_imgs = metal_imgs.reshape(
            (jax.process_count(), -1, *metal_imgs.shape[1:])
        )[jax.process_index()]
        ma_imgs = ma_imgs.reshape((jax.process_count(), -1, *ma_imgs.shape[1:]))[
            jax.process_index()
        ]
        gt_imgs = mar_data["gt_imgs"]
        gt_imgs = gt_imgs.reshape((jax.process_count(), -1, *gt_imgs.shape[1:]))[
            jax.process_index()
        ]
    else:
        mar = False
    ...
    if not use_classical_solver:
        ...
        state = checkpoints.restore_checkpoint(
            checkpoint_dir, state, step=config.eval.ckpt_id
        )
    ...
    sampling_shape = (
        config.eval.batch_size // jax.device_count(),
        config.data.image_size,
        config.data.image_size,
        config.data.num_channels,
    )

    cs_solver = cs.get_cs_solver(
        ...
        with torch.no_grad():
            if hist_norm:
                ...
                predictions = torch.clamp(
                    (predictions - peak_pred1) / (peak_pred2 - peak_pred1), min=0
                )
                targets = torch.clamp(
                    (targets - peak_targ1) / (peak_targ2 - peak_targ1), min=0
                )

                predictions = torch.clamp(
                    predictions, max=torch.max(targets).item(), min=0
                )
    ...
    for batch in tqdm.tqdm(range(num_batches)):
        if not mar:
            current_batch = (
                jnp.asarray(
                    test_imgs[
                        batch * per_host_batch_size : min(
                            (batch + 1) * per_host_batch_size, len(test_imgs)
                        )
                    ],
                    dtype=jnp.float32,
                )
            ...
            current_batch = jnp.asarray(
                ma_imgs[
                    batch * per_host_batch_size : min(
                        (batch + 1) * per_host_batch_size, len(ma_imgs)
                    )
                ],
                dtype=jnp.float32,
            )
            test_batch = jnp.asarray(
                gt_imgs[
                    batch * per_host_batch_size : min(
                        (batch + 1) * per_host_batch_size, len(gt_imgs)
                    )
                ],
                dtype=jnp.float32,
            )
            metal_batch = jnp.asarray(
                metal_imgs[
                    batch * per_host_batch_size : min(
                        (batch + 1) * per_host_batch_size, len(metal_imgs)
                    )
                ],
                dtype=jnp.bool_,
            )
        ...
        samples = samples.reshape(
            (-1, config.data.image_size, config.data.image_size, 1)
        )[:n_effective_samples]
        all_samples.extend(samples)

        if not mar:
            ground_truth = np.asarray(inverse_scaler(img)).reshape(
                (-1, config.data.image_size, config.data.image_size, 1)
            )
            ...
            for i in range(n_effective_samples):
                ...
                p, s = get_metric(
                    samples[i].squeeze(), ground_truth[i].squeeze(), mask_roi=True
                )
            ...
            # Save one example per batch (first image)
            example_path = os.path.join(
                examples_dir, f"{method_name}_batch{batch:04d}.png"
            )
            save_example_image(
                recon=samples[0].squeeze().numpy(),
                ground_truth=ground_truth[0].squeeze().numpy(),
                psnr=all_psnrs[-n_effective_samples],  # First image in this batch
                ssim=all_ssims[-n_effective_samples],
                save_path=example_path,
                method_name=method_name,
            )

            print(
                f"PSNR: {np.asarray(all_psnrs).mean():.4f}, SSIM: {np.asarray(all_ssims).mean():.4f}"
            )
            print(
                f"with mask: PSNR: {np.asarray(all_psnrs_mask).mean():.4f}, SSIM: {np.asarray(all_ssims_mask).mean():.4f}"
            )
            print(
            ...
            for i in range(n_effective_samples):
                ssim, psnr, rmse = compute_mar_metrics(
                    ground_truth[i].copy(), samples[i].copy(), masks[i].copy()
                )
            ...
            save_example_image(
                ...
                psnr=all_mar_psnrs[-n_effective_samples],  # First image in this batch
                ssim=all_mar_ssims[-n_effective_samples],
                save_path=example_path,
                method_name=method_name,
            )

            print(
                f"SSIM: {np.asarray(all_mar_ssims).mean():.4f}, PSNR: {np.asarray(all_mar_psnrs).mean():.4f}, "
                f"RMSE: {np.asarray(all_mar_rmses).mean():.4f}"
            )

    all_samples = (np.stack(all_samples, axis=0) * 255.0).astype(np.uint8)
    np.savez_compressed(
        os.path.join(eval_dir, "reconstructions.npz"), recon=all_samples
    )
    if not mar:
...
def hyperparam_search(config, workdir, eval_folder="eval"):
    ...
    # Build data pipeline
    train_ds, eval_ds, _ = datasets.get_dataset(
        config, additional_dim=None, uniform_dequantization=False, evaluation=True
    )
    ...
    if tf.io.gfile.exists(save_path):
        ...
        elif config.sampling.cs_solver.lower() in [
            "central_slice_inpainting",
            "override_inpainting",
        ]:
            ax_client.create_experiment(
        ...
        def metric_fn(hyper_params):
            ...
            if config.sampling.cs_solver.lower() in ("langevin", "baseline"):
                samples = cs_solver(
                    ...
                    pstate,
                    img,
                    hyper_params["projection_sigma_rate"],
                    hyper_params["snr"],
                )
            elif config.sampling.cs_solver.lower() in (
                "projection",
                ...
                samples = cs_solver(
                    step_rng, pstate, img, hyper_params["coeff"], hyper_params["snr"]
                )
            elif config.sampling.cs_solver.lower() in [
                "central_slice_inpainting",
                "override_inpainting",
            ...
            if not mar:
                ...
                ground_truth = np.clip(ground_truth, 0.0, 1.0)
                samples = np.clip(
                    np.asarray(samples).reshape(ground_truth.shape), 0.0, 1.0
                )
                ssims = []
                ...
                for i in range(len(random_idx)):
                    ssim, _, _ = compute_mar_metrics(
                        ground_truth[i], samples[i], metal_mask[i]
                    )
                    ssims.append(ssim)
